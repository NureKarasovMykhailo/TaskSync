...
const ActivityPage = observer(() => {
    ...
    return (
        isLoading ?
            <Container
                className={"w-100 min-vh-100 border mt-3 mb-3 d-flex justify-content-center align-items-center"}
            >
                <Loader />
            </Container>
            :
            <Container
                className={"w-100 min-vh-100 border mt-3 mb-3 p-3 "}
            >
                <div>
                    <h2>Активність</h2>
                </div>
                ...
                < hr/>
                <div>
                    <strong>Закріплені користувачі</strong>
                    <WorkerList workers={activity.users} onDelete={handleDelete} />
                    { isUserAdmin && activity.requiredWorkerCount < activity.users.length &&
